...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(origin: OriginFor<T>) -> DispatchResult {
			...
			ensure!(price + T::Currency::minimum_balance() <= T::Currency::free_balance(&who), Error::<T>::InsufficientBalance);
		...
		pub fn sale(origin: OriginFor<T>, kitty_id: KittyId) -> DispatchResult{
			...
			Self::deposit_event(Event::<T>::KittyOnSale { who: who, kitty_id: kitty_id });
			Ok(())
		}
		...
		pub fn buy(origin: OriginFor<T>, kitty_id: KittyId) -> DispatchResult{
			...
			Self::deposit_event(Event::<T>::KittyBought { who: who, kitty_id: kitty_id });
			Ok(())
		}
	}
