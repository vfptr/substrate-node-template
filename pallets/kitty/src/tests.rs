...
const KITTY_NAME: [u8; 8] = *b"12345678";
...
mod create_fun {
	...
	fn it_works_for_create() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Kitty::create(who, KITTY_NAME));
			let balance = <Test as Config>::Currency::free_balance(&account_id);
	...
	fn create_fail_when_insufficient_balance() {
		new_test_ext().execute_with(|| {
			...
			let balance = <Test as Config>::Currency::free_balance(&account_id);
			assert_eq!(init_balance, balance);
			assert_noop!(Kitty::create(who, KITTY_NAME), Error::<Test>::InsufficientBalance);
		})
	}
	...
	fn create_failed_when_kitty_id_too_large() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Kitty::create(RuntimeOrigin::signed(account_id), KITTY_NAME),
	...
	fn kitty_id_increased_after_create() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Kitty::create(RuntimeOrigin::signed(account_id), KITTY_NAME));
...
mod breed_fun {
	...
	fn breed_fail_when_insufficient_balance() {
		new_test_ext().execute_with(|| {
			...
			let _ = Kitty::create(RuntimeOrigin::signed(account_id), KITTY_NAME);
			...
			assert_noop!(
				Kitty::breed(RuntimeOrigin::signed(account_id), KITTY_NAME, kitty_id_1, kitty_id_2),
	...
	fn breed_kitty_failed_when_parents_not_exists() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Kitty::breed(RuntimeOrigin::signed(account_id), KITTY_NAME, one_parent_id, one_parent_id + 1),
				Error::<Test>::InvalidKittyId
			);
			assert_noop!(
				Kitty::breed(RuntimeOrigin::signed(account_id), KITTY_NAME, one_parent_id + 1, one_parent_id),
				Error::<Test>::InvalidKittyId
			);
		});
	}
	...
	fn breed_kitty_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Kitty::breed(RuntimeOrigin::signed(account_id), KITTY_NAME, kitty_id_1, kitty_id_2));
...
mod sale_fun {
	...
	fn sale_fail_when_not_owner() {
		new_test_ext().execute_with(|| {
			...
			let kitty_id = Kitty::next_kitty_id();
			assert_ok!(Kitty::create(saler.clone(), KITTY_NAME));
