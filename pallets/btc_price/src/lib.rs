...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		fn derived_key(block_number: T::BlockNumber) -> Vec<u8> {
			block_number.using_encoded(|_encoded_bn| ONCHAIN_TX_KEY.into_iter().copied().collect())
		}
	}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn offchain_worker(_block_number: T::BlockNumber) {
			...
			let mut number = 0;
			match btc_price::query_btc_price() {
				Ok(info) => {
					number = info.bitcoin.usd.into();
				},
				Err(_err) => {
					// log::error!("OCW ==> http reqeust error : {:?}", err);
				},
			};
